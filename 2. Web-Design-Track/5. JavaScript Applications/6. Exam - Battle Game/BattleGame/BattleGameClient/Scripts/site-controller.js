...
var SiteController = Class.create({
    ...
    renderLogInForm: function () {
        ...
        this.root.html(
            '<form>' +
                '<button id="btn-switch-to-login-form" class="selected">Login?</button>' +
                '<button id="btn-switch-to-register-form">Register?</button>' +
                '<fieldset id="login-form">' +
                    '<legend>Login</legend>' +
                    "<label for='tb-login-user'>Username: </label>" +
                    "<input type='text' id='tb-login-user' />" +
                    "<label for='tb-login-pass'>Password: </label>" +
                    "<input type='password' id='tb-login-pass' />" +
                    "<button id='btn-login'>Login</button> " +
                '</fieldset>' +
                '<fieldset id="register-form" style="display:none;">' +
                    '<legend>Register</legend>' +
                    "<label for='tb-register-user'>Username: </label>" +
                    "<input type='text' id='tb-register-user' />" +
                    "<label for='tb-register-pass'>Password: </label>" +
                    "<input type='password' id='tb-register-pass' />" +
                    "<label for='tb-register-nick'>Nickname: </label>" +
                    "<input type='text' id='tb-register-nick' />" +
    ...
    renderHitPoints: function (currentHitPoint, unitType) {
        ...
        //var missingHPContainer = $('<div class="missing-hp"></div>');
        //var missingHpPercents = 100 - currentHpPercents;
        //missingHPContainer.css('width', missingHpPercents + '%');

        hitPointsContainer.append(currentHPContainer);
        //hitPointsContainer.append(missingHPContainer);
        ...
        this.root.on('click', '.cell', function () {
            ...
            if ($('.selected-cell').length == 0 && target.hasClass(self.colorForCurrentGame + '-unit')) {
                ...
                self.service.battle.defend(gameData, function () {
                    self.service.game.field({ "gameId": gameId }, function (receivedData) {
                        self.renderGameField(receivedData);
                    }, function (receivedData) {
                        self.errorHandler(receivedData);
                    });
    ...
    errorHandler: function (receivedData) {
        ...
        if (receivedData.responseJSON) {
            if (receivedData.responseJSON.errCode === "INV_USR_AUTH") {
